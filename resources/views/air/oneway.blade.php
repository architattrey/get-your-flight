							...
							<div class="hot-page2-alp-l3 hot-page2-alp-l-com">
                                ...
                                <div class="hot-page2-alp-l-com1 hot-page2-alp-p4">	
                                    <ul style="display: inline-flex;">
                                        <li>
                                            <label class="flights departure_time_class" isflight="@{{filtering_flight('timing', 1, 0) ? 1 : 0}}" ng-click="filtering_flight('timing', 1, 0) ? filter_timing(1) : angular.noop()" id="departure_time_1" value="1">
											...
											<small class="small-txt" ng-if="filtering_flight('timing', 1, 0)"><i class="fa fa-inr"></i> @{{filter_min_price("timing", 1, 0)}}</small>
                                        </li>
                                        <li>
                                            <label class="flights  departure_time_class" isflight="@{{filtering_flight('timing', 2, 0) ? 1 : 0}}" ng-click="filtering_flight('timing', 2, 0) ? filter_timing(2) : angular.noop()" id="departure_time_2" value="2">
											...
											<small class="small-txt" ng-if="filtering_flight('timing', 2, 0)"><i class="fa fa-inr"></i> @{{filter_min_price("timing", 2, 0)}}</small>
                                        </li>
                                        <li>
                                            <label class="flights departure_time_class" isflight="@{{filtering_flight('timing', 3, 0) ? 1 : 0}}" ng-click="filtering_flight('timing', 3, 0) ? filter_timing(3) : angular.noop()" id="departure_time_3" value="3">
											...
											<small class="small-txt" ng-if="filtering_flight('timing', 3, 0)"><i class="fa fa-inr"></i> @{{filter_min_price("timing", 3, 0)}}</small>
                                        </li>
                                        <li>
                                            <label class="flights departure_time_class" isflight="@{{filtering_flight('timing', 4, 0) ? 1 : 0}}" ng-click="filtering_flight('timing', 4, 0) ? filter_timing(4) : angular.noop()" id="departure_time_4" value="4">
                                                <a href="#" class="departure_time_class @{{filtering_flight('timing', 4, 0) ? '' : 'btn-is-disabled'}}" value="4" title="Nights-flights" style="margin: 0;border: none; padding: 0;">
                                                    ...
                                                    <p class="departure_time_class" value="4" style="margin: 0; font-size: 10px;text-align: center; color:#000;" >18-00</p>
                                                </a>
                                                <!-- <input type="checkbox" ng-checked="true" ng-click="filter_timing(4)"> -->
                                            </label>
											<small class="small-txt" ng-if="filtering_flight('timing', 4, 0)"><i class="fa fa-inr"></i> @{{filter_min_price("timing", 4, 0)}}</small>
                                        </li>
                                    </ul>
                                </div>
							</div>
										...
										<div class="hot-page2-alp-l-com1 hot-page2-alp-p4" >
											...
											<ul style="display:inline-flex; display:inline-flex; width:60%; height:auto; margin-left:50px;" >
												<li style = "width: 50px; border" >
													<a href="#" ng-click="filter_stops(1)" isflight="@{{filtering_flight('stops', 1, 0) ? 1 : 0}}" class="stops_color stops_class @{{filtering_flight('stops', 1, 0) ? '' : 'btn-is-disabled'}}"  style="border-radius: 6px 6px 6px 6px; padding: 10px 17px; cursor: pointer;"><span>0</span></a>
													<small class="small-txt" ng-if="filtering_flight('stops', 1, 0)"><i class="fa fa-inr"></i> @{{filter_min_price("stops", 1)}}</small>
													<!-- <label>0
												...
												<li style = "width: 50px;" >
													...
													<small  class="small-txt" ng-if="filtering_flight('stops', 2, 0)"><i class="fa fa-inr"></i> @{{filter_min_price("stops", 2)}}</small>
													<!-- <label>1
														<input type="checkbox" ng-checked="true" ng-click="filter_stops(2)">
													</label> -->
												</li>
												<li >
													<a href="javascript:void(0)" isflight="@{{filtering_flight('stops', 3, 0) ? 1 : 0}}"   ng-click="filter_stops(3)" class="stops_color stops_class @{{filtering_flight('stops', 3, 0) ? '' : 'btn-is-disabled'}}"
													 style="border-radius: 6px 6px 6px 6px; padding: 10px 13px;cursor: pointer;"><span>2+</span></a>
													<small class="small-txt" ng-if="filtering_flight('stops', 3, 0)"><i class="fa fa-inr"></i> @{{filter_min_price("stops", 3)}}</small>
													<!-- <label>2+
								...
								<div class="hot-page2-alp-l-com1 hot-page2-alp-p4"  style="height:100px">
									<ul>
                                        ...
                                        <input type="checkbox" > -->
											...
											<span class="checkmark" style=" margin: 5px 0px 0px -115px;"></span>
										</label>
										<small class="small-txt" ng-if="filtering_flight('isrefundable', 1, 0)"><i class="fa fa-inr"></i> @{{filter_min_price("isrefundable", 1)}}</small>
										</li>
										<li  style="width: 100%; float: left; padding: 3px 0;">								
											<label class="containerbox" style="border:0px solid grey; font-family:'Poppins',sans-serif; font-size: 14.5px; line-height: 24px;font-family: 'Poppins', sans-serif; font-weight: 400;  display: inline;">
											<input  checked="checked" type="checkbox" ng-checked="true" ng-click="filter_refundable(2)"> &nbsp;  Non Refundable
											<span class="checkmark" style=" margin: 5px 0px 0px -147px;"></span>
											</label>
											<small class="small-txt" ng-if="filtering_flight('isrefundable', 2, 0)"><i class="fa fa-inr"></i> @{{filter_min_price("isrefundable", 2)}}</small>
										</li>
									</ul>
								</div>
							</div>
                                        ...
                                        <li ng-repeat="airlines_name in airlinnameshow">
                                            <label class="containerbox" style="border:0px solid grey; font-family:'Poppins',sans-serif;font-size: 14.5px; line-height: 24px;font-family: 'Poppins', sans-serif; font-weight: 400;"> 
												<div class="row">
													...
													<div class="col-md-8">
														@{{airlines_name}} <small  class="small-txt"><i class="fa fa-inr"></i> @{{filter_min_price("airlines", airlines_name, 0)}}</small>
													</div>
													<div class="col-md-2">
											...
											<div class="col-md-12" style="margin: 0;padding: 0;">									
												<table id="myTable">
													<tbody style=" background-color: #fff;">
														<tr>
															<th class="e_h1"><a href="#">Airline</a></th>
															<th class="e_h1"><a href="#">Departure</a></th>
															<th class="e_h1"><a href="#">Arrival</a></th>
															<th class="e_h1"><a href="#">Duration</a></th>
															<th class="e_h1"><a href="#">Price</a></th>
															<th>Book</th>
														</tr>
																...
																<div class="row">
																	<div class="col-md-8 col-md-offset-2">
																		<div class="col-md-3 col-sm-3 col-xs-3">
																			<img  ng-src="http://tbolite.tektravels.com/Images/Air/AirlineLogo_25x25/@{{flight.AirlineCode}}.gif"/>
																		</div>
																		<div class="col-md-9 col-sm-9 col-xs-9 pull-left">
																			@{{flight.airlineName}}<br><span style="font-size: 11px;">@{{flight.AirlineCode}} - @{{flight.Segments[0][0].Airline.FlightNumber}}</span>
																		</div>
																	</div>
																</div>

																<div class="row" ng-if="flight.meal == 'Free Meal'">
																	Free meal
																</div>

															</td>
															<td>
																<span><b>@{{flight.Segments[0][0].StopPointDepartureTime | date: 'HH:mm'}}</b></span>	
																<small style="font-size: 11px !important; font-size: 11px !important; display: block;">@{{origin}}</small>
															</td>
															<td>
																<span><b>@{{flight.Segments[flight.Segments.length - 1][0].StopPointArrivalTime | date: 'HH:mm'}}</b></span>	
																<small style="font-size: 11px !important; font-size: 11px !important; display: block;">{{$destination}}</small>
															</td>
															<td><div><span style="display: inline-block; ">@{{all_duration(flight.Segments, 1)}}</span><small style="font-size:11px;"><br>@{{flight.stops == 1 ? 'Non Stop' : flight.stops - 1 + ' Stop'}}</small></div></td>
															<td>
																<span class="rupye"><i class="fa fa-inr" style=""></i><b>@{{flight.price}}</b></span>	
																<small style="font-size: 11px !important; font-size: 11px !important; display: block;">@{{flight.IsRefundable ? 'Refundable' : 'Non Refundable'}}</small>
															</td>
															<td>
																<a herf="@{{booking_form(flight.ResultIndex)}}"  style="" class="link-btn book_ticket">Book Now</a><br>
	...
	<script>
	    ...
	    appdata.controller('flightCtrl', function($scope, $http) {
			...
			$scope.book_link_return = (from_index, return_index) => {
				var book_link_return = "{{url('/')}}/"+ from_index + '-' + return_index + '/' + $scope.traceId+"?adult={{$adult}}&child={{$child}}&infant={{$infant}}"
